...
{
    class VaultBackupPolicyInventory : AzureInventory<VaultBackupPolicyModel>
    {
        public VaultBackupPolicyInventory(AzureService azureService, IRepository<VaultBackupPolicyModel> repository) : 
            base(azureService, repository, "backup policy", "Create Protection Policy") { }

        public override async Task<IEnumerable<VaultBackupPolicyModel>> GetInventoryAsync(IEnumerable<VaultBackupPolicyModel> inventoryParam = null)
        {
            ...
            var inventory = new List<VaultBackupPolicyModel>();
                    ...
                    foreach (var vault in vaults) {
                        try {
                            ...
                            foreach (var policy in policies) {
                                try {
                                    var newPolicy = new VaultBackupPolicyModel();
                                    ...
                                    if (policy.Properties.GetType().Name == "AzureIaaSVMProtectionPolicy") {
                                        ...
                                        if (properties.SubProtectionPolicy.Count != 2) {
                                            throw new Exception(
                                                $"Encountered {properties.SubProtectionPolicy.Count} SubProtection policies. Expected only two. " +
                                                "Please review and add support in Azure dashboard.");
                                        }

                                        if (properties.SubProtectionPolicy[0].PolicyType != "Full") {
                                            throw new Exception(
                                                $"Encountered '{properties.SubProtectionPolicy[0].PolicyType}' as the first SubProtectionPolicy type. " +
                                                "Expected 'Full'. Please review and add support in Azure dashboard.");
                                        }

                                        if (properties.SubProtectionPolicy[1].PolicyType != "Log") {
                                            throw new Exception(
                                                $"Encountered '{properties.SubProtectionPolicy[1].PolicyType}' as the second SubProtectionPolicy type. " +
        ...
        public override async Task<IEnumerable<VaultBackupPolicyModel>> ProcessInventoryAsync(
            IEnumerable<VaultBackupPolicyModel> inventoryParam)
        {
            ...
            var itemsToInsert = new List<VaultBackupPolicyModel>();
            var itemsToUpdate = new List<VaultBackupPolicyModel>();
        ...
        private void SetLongTermRetentionPolicyProperties(LongTermRetentionPolicy retentionPolicy, VaultBackupPolicyModel newPolicy)
        {
        ...
        private void SetSimpleRetentionPolicyProperties(SimpleRetentionPolicy retentionPolicy, VaultBackupPolicyModel newPolicy)
        {
            if (retentionPolicy == null) {
                return;
            }
        ...
        private void SetSimpleSchedulePolicyProperties(SimpleSchedulePolicy schedulePolicy, VaultBackupPolicyModel newPolicy)
        {
        ...
        private void SetLogSchedulePolicyProperties(LogSchedulePolicy schedulePolicy, VaultBackupPolicyModel newPolicy)
        {
            if (schedulePolicy == null) {
                return;
            }
