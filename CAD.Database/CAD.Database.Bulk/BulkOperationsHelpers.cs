...
namespace CAD.Database.Bulk
{
    internal class BulkOperationsHelpers
    {
        ...
        internal string BuildJoinConditionsForUpdateOrInsert(string[] updateOn, string sourceAlias, string targetAlias)
        {
            ...
            if (updateOn.Length > 1) {
                ...
                for (int i = 1; i < updateOn.Length; i++) {
                    command.Append("AND " + "[" + targetAlias + "]" + "." + "[" + updateOn[i] + "]" + " = " + "[" + sourceAlias + "]" + "." + "[" +
                                   updateOn[i] + "]" + " ");
                }
            }
        ...
        internal DataTable ToDataTable<T>(IEnumerable<T> items, HashSet<string> columns, Dictionary<string, string> columnMappings,
            List<string> matchOnColumns = null, bool? outputIdentity = null, Dictionary<int, T> outputIdentityDic = null)
        {
        ...
        internal string GetFullQualifyingTableName(string databaseName, string schemaName, string tableName)
        {
            ...
            if (!databaseName.StartsWith("[")) { sb.Append("["); }
            sb.Append(databaseName);
            if (!databaseName.EndsWith("]")) { sb.Append("]"); }
            sb.Append(".");
            if (!schemaName.StartsWith("[")) { sb.Append("["); }
            sb.Append(schemaName);
            if (!schemaName.EndsWith("]")) { sb.Append("]"); }
            sb.Append(".");
            if (!tableName.StartsWith("[")) { sb.Append("["); }
            sb.Append(tableName);
            if (!tableName.EndsWith("]")) { sb.Append("]");}
        ...
        internal void SetSqlBulkCopySettings(SqlBulkCopy bulkcopy, bool bulkCopyEnableStreaming, int? bulkCopyBatchSize, int? bulkCopyNotifyAfter,
            int bulkCopyTimeout)
        {
        ...
        internal void InsertToTmpTable(SqlConnection conn, SqlTransaction transaction, DataTable dt, bool bulkCopyEnableStreaming, int? bulkCopyBatchSize,
            int? bulkCopyNotifyAfter, int bulkCopyTimeout, SqlBulkCopyOptions sqlBulkCopyOptions)
        {
